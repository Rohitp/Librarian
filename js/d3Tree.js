var dndTree = (function() {
    ...
    function setChildrenAndUpdate(node) {
        ...
        var books;
        console.log(node);
        AE.getRelated(node.book, exploredIds).then(function(books) {
            if (!node.children) {
                node.children = []
            }
    ...
    function initWithBook(book) {
        exploredIds.push(book.id.toString());
        return {
            'book' : book,
            'children': null,
        }
            ...
            .attr("xlink:href", function(d) {
              console.log(d)
                ...
                /*if (!posters) {
                 ...
                 remove
                if (isArtist(d)) {
                  return AE.getImage(d.artist.images);
                } else {
    ...
    return {
        "setRootBook" : function(book) {
           exploredIds = []
           root = initWithBook(book);
